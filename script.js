...
let palabrasImportantes = new Set([
    ...
    "Dios", "señor", "Jesús", "cristo", "espíritu", "padre", "hijo", "paráclito", "mesías", "trinidad", "creador", "todopoderoso",

    // Conceptos Teológicos y Sacramentales
    "amor", "fe", "gracia", "salvación", "pecado", "justicia", "misericordia", "vida", "muerte", "resurrección", "redención", "santidad", "gloria", "paz", "sabiduría", "verdad", "luz", "oscuridad", "misterio", "providencia",

    // Historia de la Salvación y Alianza
    "reino", "palabra", "alianza", "promesa", "ley", "mandamiento", "evangelio", "testimonio", "cruz", "sangre", "cuerpo", "sacrificio", "pascua", "pentecostés", "éxodo", "exilio",

    // Figuras y Vocaciones
    "hombre", "mujer", "ángel", "profeta", "apóstol", "discípulo", "sacerdote", "rey", "virgen", "maría", "pedro", "pablo", "juan", "david", "abraham", "moisés", "israel",

    // Prácticas y Vida de la Iglesia
    "oración", "perdón", "iglesia", "bautismo", "eucaristía", "caridad", "esperanza", "ayuno", "limosna", "tentación", "obediencia", "humildad", "compasión", "temor", "paciencia", "pureza", "fidelidad",

    // Lugares Bíblicos Clave
    "cielo", "tierra", "sinaí", "jerusalén", "sión", "templo", "tabernáculo", "arca", "desierto", "pueblo", "horeb"
]);
...
function mostrarNotasPersonales(libro, capitulo, versiculo) {
    ...
    if (notas.length === 0) {
        container.innerHTML = '<div class="text-gray-400 italic text-center py-4">No hay notas personales para este versículo.</div>';
        return;
    }
    container.innerHTML = notas.map((nota, idx) => {
        const esFav = esFavoritoComentario(libro, capitulo, versiculo, 'personal', idx);
        return `
        <div class="border-l-4 border-oro/30 pl-4 py-3 bg-gray-50 dark:bg-gray-800/40 rounded-lg mb-3">
            <p class="text-gray-700 dark:text-gray-300 text-sm mb-2">${escapeHtml(nota.texto)}</p>
            <div class="flex justify-between items-center mt-2 border-t border-gray-200 dark:border-gray-700 pt-2">
                <span class="text-[10px] text-gray-400 uppercase font-sans">${nota.fecha}</span>
...
function buscarVersiculos(termino) {
    ...
    const termNormalizado = normalizarTexto(termino);
    const termLimpio = termNormalizado.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex = new RegExp(`\\b${termLimpio}\\b`, 'gi');
...
function inicializarIndice() {
    ...
    for (const [testamento, libros] of Object.entries(canonBiblico)) {
        ...
        libros.forEach(libro => {
            ...
            btn.className = "text-left py-2 px-3 rounded text-lg transition-all hover:bg-yellow-50 dark:hover:bg-gray-800 hover:text-oro hover:border-l-4 hover:border-oro bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm";
            btn.onclick = () => abrirCapitulos(libro.nombre, libro.caps);
...
function abrirPrefacio(libro) {
    ...
    const panelHtml = `
        ...
        <div class="text-xs font-sans text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2"><i class="fas fa-feather-alt"></i> Tradición sobre el Evangelio</div>
    ` + (comentarios.length === 0 ? `<div class="text-gray-400 italic font-sans text-center py-10">✨ Comentarios del prefacio en preparación.</div>` : comentarios.map((c, idx) => {
...
function abrirCapitulos(nombreLibro, cantidadCapitulos) {
    ...
    for (let cap of listaCaps) {
        const btn = document.createElement('button');
        btn.textContent = cap;
        btn.className = `py-3 rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md hover:border-oro hover:text-oro transition-all font-sans font-bold text-base`;
        btn.onclick = (function(c) { return function() { abrirLectura(c); }; })(cap);
    ...
    if (comentarios.length === 0) {
        ...
        tradicionHtml += comentarios.map((c, idx) => {
            ...
            <div class="border-l-4 border-oro/30 pl-4 py-2">
                ...
                <p class="text-gray-700 dark:text-gray-300 text-sm mt-2">${escapeHtml(c.texto)}</p>
                <button onclick="compartirComentario('${libro}', ${capitulo}, ${versiculo}, '${escapeHtml(c.autor)}', '${escapeHtml(c.texto)}')" class="mt-2 text-xs text-oro hover:underline flex items-center gap-1"><i class="fas fa-share-alt"></i> Compartir</button>
            </div>
            `;
        }).join('');
    }
